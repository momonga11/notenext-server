...
RSpec.describe 'Folders', type: :request do
  ...
  describe 'POST #create' do
    ...
    it '認証されていない場合は作成できない' do
      auth_headers['access-token'] = '12345'
      post v1_project_folders_path(project_id: user.projects[0].id), params: { folder: folder_attributes }, headers: auth_headers
      expect(response). to have_http_status(401)
    end
    ...
    context '認証されている場合' do
      ...
      it 'ユーザーが所属していないプロジェクトの場合、フォルダは作成できない' do
        expect do
          post v1_project_folders_path(project_id: user_dummy.projects[0].id), params: { folder: folder_attributes }, headers: auth_headers
      ...
      context 'パラメーターが異常値の場合' do
        it '作成できない' do
          post v1_project_folders_path(project_id: user.projects[0].id), params: { test: 'test' }, headers: auth_headers
  ...
  describe 'PUT #update' do
    ...
    it '認証されていない場合は更新できない' do
      auth_headers['access-token'] = '12345'
      put v1_project_folder_path(project_id: folder.project.id, id: folder.id), params: { folder: folder2_attributes }, headers: auth_headers
      expect(response). to have_http_status(401)
    end
    ...
    context '認証されている場合' do
      ...
      it 'ユーザーが所属しないプロジェクトの場合は更新できない' do
        put v1_project_folder_path(project_id: user_dummy.projects[0].id, id: folder_dummy.id), params: { folder: folder2_attributes }, headers: auth_headers
        expect(response.status).to eq(403)
      ...
      context 'パラメーターが異常値の場合' do
        it '更新できない' do
          put v1_project_folder_path(project_id: folder.project.id, id: folder.id), params: { test: 'test' }, headers: auth_headers
          expect(response). to have_http_status(400)
        end
        ...
        it '存在しないIDの場合は、更新できない' do
          put v1_project_folder_path(project_id: folder.project.id, id: -1), params: { folder: folder2_attributes }, headers: auth_headers
          expect(response). to have_http_status(404)
        end
      end
    end
